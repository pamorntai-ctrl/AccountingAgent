...
from tools.google_sheet_expense import append_expense_row
...
expense_agent = Agent(
    name="ExpenseRecordingAgent",
    model="gemini-2.5-flash",
    description="Agent responsible for recording expense transactions into Google Sheets",

    instruction="""
You are an Expense Recording Agent.

Your job is to record expense transactions into Google Sheets.

You will receive structured expense data extracted from receipt or invoice documents.

Your tasks:
1. Validate that the data contains basic expense information.
2. Use the append_expense_row tool to save the data into Google Sheets.
3. Do not modify or reinterpret the provided data.
4. Do not analyze documents or images.
5. Only store the provided structured data.
...
After saving the record, confirm that the expense has been recorded.
""",

    tools=[append_expense_row]
)
